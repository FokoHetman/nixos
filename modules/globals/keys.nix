_: {
  flake.globals = {
    fokossh = [
    "ssh-rsa  foko@hetman.at"
    ];
    nathanssh = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFs8Toyc7bQ9n6LV7czYtpCj6Ki5hItivcuWY21+iPfo nathan@nathanpc"
    ];
    toastssh = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIM3xlUW2U02zcGS++Z/GIK6nda3t/e46y4u39CBhpmas toast@laptop"
    ];
    flake.globals.solinassh = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIMfy0gxynalzYKxPikTFN2WDzTE+MkkBlrXXmnNDsIzd"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIO3VfMgbZLZgtKddz077uZrxI40qiLWuw9SSRqJXirgu"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAICEQcJcMauLgMg7mG2R8QvhwEPThjJZ7BX6bqX+0859K"
    ];
  };
}
