...
fname=$(${pkgs.zenity}/bin/zenity --entry --title="GIF Name" --text="Provide the name for the recorder GIF.")
...
selectRegion() {
    ${pkgs.slop}/bin/slop --format "%w %h %x %y"
}
...
${ffmpegxcb}/bin/ffmpeg -y -f x11grab -s "$W"x"$H" -i :0.0+$X,$Y "$tmpfile_mkv"
expectedExitCode=0

${ffmpegxcb}/bin/ffmpeg -y -i "$tmpfile_mkv" -vf fps=10,palettegen "$palette_png"

${pkgs.libnotify}/bin/notify-send 'Generating GIF.'
${ffmpegxcb}/bin/ffmpeg -y -i "$tmpfile_mkv" -i "$palette_png" -filter_complex "paletteuse" $output.gif

mv $tmpfile_mkv $output.mkv
echo "$output"
${pkgs.libnotify}/bin/notify-send 'Saved at $output.gif.'
